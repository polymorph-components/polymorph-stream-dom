//! TodoMVC as a `polymorph:stream-dom` producer.
//!
//! Ported from polyengine-dioxus's `examples/todomvc/src/lib.rs`, which is
//! itself DioxusLabs/dioxus @ v0.7.10
//! `examples/01-app-demos/todomvc.rs` (MIT/Apache-2.0) with the changes that
//! example's own header records: no `asset!`/`Stylesheet` node (the demo page
//! links todomvc-app-css directly), a lib crate exporting a wasm component
//! rather than a `main`, and `values_mut()` in `toggle_all` to satisfy
//! `clippy::for_kv_map`.
//!
//! The only change here on top of that: `stream_dom_dioxus::launch!` in place
//! of `polyengine_dioxus::launch!`. Components, names and structure are
//! untouched so this keeps proving the *real* example runs on this producer.
